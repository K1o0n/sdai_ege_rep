...
def add_user(
        data):  # dict (name, surname, patronymic, email, password, telephone, age, country, role (user, user, admin), about, path (path to the photo))
    current = [data['name'], data['surname'], data['patronymic'], data['email'], data['password'], data['telephone'],
               data['age'], data['country'], data['role'], data['about'], data['path'], time(), 1]
    make_interferation(
...
def add_attempt(data):  # dict: (ID_user, ID_task, is_right, ID_course)
    current = [data['ID_user'], data['ID_task'], data['is_right'], data['answer'], time(), data['ID_result'],
               data['ID_course']]
...
def get_groups_for_teacher(teacher_id):
    result = make_request(
        "SELECT Groups.ID, Groups.name FROM Groups JOIN Groups_Teachers ON Groups.ID = Groups_Teachers.ID_group WHERE ID_user = ?",
        [teacher_id])
...
def get_groups_for_student(student_id):
    result = make_request(
        "SELECT Groups.ID, Groups.name FROM Groups JOIN Groups_Students ON Groups.ID = Groups_Students.ID_group WHERE ID_user = ?",
        [student_id])
    return result  # [(id, name)]
...
def get_options_for_group(id_group):
    result = make_request(
        "SELECT Oprions.ID, Options.name, Options.ID_user FROM Options JOIN Groups_Options ON Groups_Options.ID_option = Option.ID WHERE ID_group = ?",
        [id_group])
    return result  # [(id, name, id_of_creator)]


def get_students_for_group(id_group):
    result = make_request("SELECT ID_student FROM Group_Students WHERE ID_group = ?", [id_group])
    return result


def get_teachers_for_group(id_group):
    result = make_request("SELECT ID_teacher FROM Group_Teachers WHERE ID_group = ?", [id_group])
    return result
...
def get_user_role(id_user, status):
    result = make_request("SELECT role FROM Users WHERE id = ? AND status = ?", [id_user, status])
    return result[0][0]
...
def get_results_for_option_user(option_id, user_id):
...
def get_tasks_for_course(id_course):
    result = make_request(
        "SELECT * FROM Course_Tasks JOIN Tasks ON Tasks.ID = Course_Tasks.ID_task WHERE ID_course = ?", [id_course])
...
def change_user(id_user,
                data):  # list or dict (name, surname, patronymic, email, password, telephone, age, country, role (user, user, admin), about, path (path to the photo))
    current = [data['name'], data['surname'], data['patronymic'], data['email'], data['password'], data['telephone'],
               data['age'], data['country'], data['role'], data['about'], data['path'], 1]
    current += [id_user]
    make_interferation(
...
def change_task(id_task, data):  # dict: (text, answer, difficulty, num_in_ege, source)
    ...
    current += [id_task]
    make_interferation(
        "UPDATE Tasks SET text=?, answer=?, difficulty=?, ID_type=?, source=?, date=?, status=? WHERE ID = ?", current)
