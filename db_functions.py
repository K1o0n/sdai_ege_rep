...
def get_students_for_group(group_id):
    ...
    """
    result = make_request("SELECT Users.id, name, surname FROM Users JOIN Groups_Students ON Users.id = Groups_Students.ID_user WHERE ID_group = ?", [group_id])
    return result
...
def get_teachers_for_group(group_id):
    ...
    :return: List[Tuple(id:int, name:str, surname:str)]
    """
    result = make_request("SELECT Users.id, name, surname FROM Users JOIN Groups_Teachers ON Users.id = Groups_Teachers.ID_user WHERE ID_group = ?", [group_id])
    return result
...
def get_user(user_id, status):
    ...
    result = make_request("SELECT * FROM Users WHERE id = ? AND status = ?", [user_id, status])
...
def get_user_role(user_id, status):
    '''int user_id, int status (1-active, 2-banned, 3-deleted)'''
    result = make_request("SELECT role FROM Users WHERE id = ? AND status = ?", [user_id, status])
    return result[0][0]
...
def get_task(task_id, status):
    ...
    result = make_request("SELECT * FROM Tasks WHERE ID = ? AND status = ?", [task_id, status])
...
def get_cnt_tasks_for_user(user_id, status):
    result = make_request("SELECT DISTINCT ID_task FROM Attempts JOIN Users ON Attempts.ID_user = Users.ID WHERE is_correct = 1 AND ID_User = ? AND status = ?", [user_id, status])
    return result

def get_cnt_types_for_user(user_id):
    result = make_request("SELECT DISTINCT ID_type FROM Attempts JOIN Tasks ON Attempts.ID_task = Tasks.ID WHERE is_correct = 1 AND ID_User = ?",[user_id])
    return result

def get_tasks_by_type(type_id, status):
    '''int type_id, int status (1-active, 2-banned, 3-deleted)'''
    result = make_request("SELECT * FROM Tasks WHERE ID_type = ? AND status = ?", [type_id, status])
    return result  # [(id, text, answer, difficulty, ID_type, source, solution, status)]
...
def get_tasks_for_course(course_id):
    '''int course_id'''
    result = make_request(
        "SELECT * FROM Course_Tasks JOIN Tasks ON Tasks.ID = Course_Tasks.ID_task WHERE ID_course = ?", [course_id])
...
def get_attempts_of_user_task(user_id, task_id):
    ...
    result = make_request("SELECT * FROM Attempts WHERE ID_user = ? AND ID_task = ? ORDER BY date", [user_id, task_id])
